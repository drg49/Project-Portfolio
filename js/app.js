...
const setTheme = (x) => {
  ...
  if (theme === "1") {
    ...
    $("#turn-blue, #about-section, #contact-heading").css(
      "background-image",
  ...
  if (theme === "2") {
    ...
    $("#turn-blue, #about-section, #contact-heading").css(
      "background-image",
      "linear-gradient(90deg, rgba(58,27,152,1) 25%, rgba(46,2,172,1) 71%)"
    );
    $("#name, #greeting, #full-stack").css("color", "white");
    $("#fet, #bet").css("color", "white");
    $("#main-heading, #about, #contact").css("color", "whitesmoke");
  ...
  if (theme === "3") {
    ...
    $("#turn-blue, #about-section, #contact-heading").css(
      "background-image",
      "linear-gradient(90deg, rgba(0,0,0,1) 8%, rgba(0,0,0,1) 71%)"
    );
  ...
  if (theme === "4") {
    ...
    $("#turn-blue, #about-section, #contact-heading").css(
      "background-image",
      "linear-gradient(90deg, rgba(0,150,111,1) 0%, rgba(21,114,65,1) 71%)"
    );
...
const projects = [
  {
    description:
      "In 2022, I began developing my first 3D video game titled <i>The Movie Theater</i>. The project took over a year to complete and was programmed using C# in Unity3D. Windows/PC users can download it for free at neptunegamestudios.com",
  ...
  {
    description:
      "Fun, tricky, and short quizzes made with HTML, CSS, and JavaScript.",
...
projects.forEach((project) => {
  const $div = $("<div>");
  $div.html(
    `<project-card projectName="${project.projectName}" img="${project.image}" description="${project.description}" github="${project.giturl}" liveurl="${project.liveurl}"></project-card>`
  );
  $main.append($div);
});
